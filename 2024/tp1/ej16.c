...
int main() {            /* Estrategia: usaremos un 'for' que recorra desde 'z' ó 'Z' hasta  
    ...
    if(letra>=65 && letra<=90){   // Si es mayúscula
        for(int i='Z'; i>=letra; i--){ // Notar el uso de los caracteres en los índices
            printf("%c ", i);
        }
    } else if(letra>=97 && letra<=122){   // Si es minúscula
        for(int i='z'; i>=letra; i--){
            printf("%c ", i);
        }
    } else {
